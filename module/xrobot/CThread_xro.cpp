...
/////////////////////////////////////////////////////////////////////////////////////////////////////////
void* CThread_xro::process_xro(void *)
{
    ...
    pxro_serial = new CSerial();
    if(NULL == pxro_serial)
    {
        return (void *)-1;
    }
    pxro_serial->openPort('1');
    pxro_serial->setPort(0, 115200, '8', '0', '0', '0', '0', '1');
    pxro_serial->uTtyDelay = 10*1000000/115200;
    ...
    while(1)
    {
        ...
        //
        FD_ZERO(&fs_read);
        FD_SET(pxro_serial->m_nFdcom, &fs_read);
        FD_SET(0, &fs_read);
        //cout << "Process Xro 001" << endl;
        fs_sel = select(pxro_serial->m_nFdcom + 1, &fs_read, NULL, NULL, NULL);
        if(-1 == fs_sel)
        {
            ; //TODO
        }
//cout << "Process Xro 001" << endl;
        if(FD_ISSET(pxro_serial->m_nFdcom, &fs_read))
        {
            pxro_serial->getDataFromSerial();

            int decode_ret = decode_xro(pxro_serial->m_chrRecvBuffer, pxro_serial->m_nRecvLen, px_msg_struct);
            //#ifdef PRINT_XRO_DEBUG_INFO
            if(decode_ret < 0)

            {
                printf("Recvd form Xrobot:");
                printf("len=%d, Decode return value is %d, update = %d\n", pxro_serial->m_nRecvLen, decode_ret, px_msg_struct->update);
                for(i = 0; i < pxro_serial->m_nRecvLen; i++)
                    printf("%2x  ", pxro_serial->m_chrRecvBuffer[i]);
                std::cout << std::endl;
        ...
        if(xro_cmd_update)
        {
            int ret_val =  pxro_serial->sendPort(xro_cmd_string, 34);
            if(ret_val == 34)
            {
...
/////////////////////////////////////////////////////////////////////////////////////////////////////////
void CThread_xro::create(pthread_t &tid,pthread_attr_t *attr,int sock)
{
    if(pthread_create(&tid,attr,process_xro,&sock))
    {
...
/////////////////////////////////////////////////////////////////////////////////////////////////////////
void CThread_xro::exit(int)
{
    pthread_exit(0);
}

/////////////////////////////////////////////////////////////////////////////////////////////////////////
int_32 CThread_xro::decode_xro(uint_8 *buffer, int_8 rcv_len, pxrobot_msg msg)
{
    int i = 0;
    int j = 0;
    uint_8 buf[64];
...
int_32 CThread_xro::print_xrobot(pxrobot_msg msg)
{
    if(msg)
    {
        /*
